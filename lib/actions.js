...
export async function getRoutesPages(query) {
  ...
  const { data: routes, error } = await supabase.from('routes').select('title');
  // .eq('title', queryItem);
  if (error) {
    console.log(error);
...
// export async function getRoutesPerPage(query) {
//   let page = 1;
//   let routesPerPage = 6;
